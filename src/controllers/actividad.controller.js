...
const obtenerActividadPorId = async (req, res) => {
  try {
    const actividad = await Actividad.findById(req.params.id)
      .populate('destino', 'nombre ubicacion')
      .populate('creadoPor', 'nombre email');
    if (!actividad || !actividad.activo) return res.status(404).json({ message: 'Actividad no encontrada.' });
    res.json({ actividad });
  } catch (error) {
...
const crearActividad = async (req, res) => {
  try {
    const actividad = await Actividad.create({ ...req.body, creadoPor: req.usuario._id });
...
const actualizarActividad = async (req, res) => {
  try {
    const actividad = await Actividad.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
...
const eliminarActividad = async (req, res) => {
  try {
    const actividad = await Actividad.findByIdAndUpdate(req.params.id, { activo: false }, { new: true });
    if (!actividad) return res.status(404).json({ message: 'Actividad no encontrada.' });
