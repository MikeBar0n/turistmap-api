...
const obtenerDestinoPorId = async (req, res) => {
  try {
    ...
    if (!destino || !destino.activo) return res.status(404).json({ message: 'Destino no encontrado.' });
    res.json({ destino });
  } catch (error) {
...
const crearDestino = async (req, res) => {
  try {
    const destino = await Destino.create({ ...req.body, creadoPor: req.usuario._id });
...
const actualizarDestino = async (req, res) => {
  try {
    const destino = await Destino.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
...
const eliminarDestino = async (req, res) => {
  try {
    const destino = await Destino.findByIdAndUpdate(req.params.id, { activo: false }, { new: true });
    if (!destino) return res.status(404).json({ message: 'Destino no encontrado.' });
